...
class DropBlock2D(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = 1 - mask
        mask = mask[:, :, :x.shape[2], :x.shape[3]]  # crop in geval van mismatch
...
class SECOND(nn.Module):
    def __init__(self,
        ...
        self.dropblocks = []  # om achteraf epoch updates te kunnen doen
        ...
        for i, num_layers in enumerate(layer_nums):
            layers = []
            use_dropblock = i >= 1  # alleen stage 2 en 3

            # Eerste conv met stride
            layers += [
